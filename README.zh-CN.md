...
上游 CrossPoint 的主要阅读能力（EPUB 2/3 渲染、多格式支持、无线传书、OPDS 等）在 CrossMux 中同样可用。联网 OTA 会检查 `ryokun6/crossmux` 的最新 GitHub Release，并为中文固件选择 `firmware-cn.bin`。也可通过 USB、网页刷机器或 SD 卡安装 `.bin` 固件。详见 [English README](./README.md)。
